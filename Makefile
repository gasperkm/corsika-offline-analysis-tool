...
LIBSO=-lRecEventKG -lAnalysisKG -lAugerEventIO -lAugerFramework -lAugerModules -lAugerTools -lAugerUtilities

#CFILES=$(shell find -maxdepth 1 -name "*.cc")
CFILES=$(SRCDIR)/analysis_tool.cc $(SRCDIR)/massanalyse.cc $(SRCDIR)/adstanalyse.cc

all: analysis_tool usage histogram_replot replot_usage
...
clean:
	rm -f analysis_tool $(IDIR)/workstation.h $(IDIR)/OfflineInclude.h histogram_replot
